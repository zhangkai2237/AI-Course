...
const slides = [
  ...
  {type:"question", title:"Demo跑通，为什么还不能上线？", lead:"因为一次漂亮回答，无法代表真实用户、真实边界和真实版本。", items:["换一种问法，还能答对吗？","用户不给完整信息，会发生什么？","知识库、Prompt或模型更新后，结果会不会退化？"], pages:"14–16、25"},
  {type:"statement", title:"一次正确，只能证明这一次正确", lead:"AI输出具有概率性；产品要验证的是一组场景下的稳定表现。", accent:"从单点演示，转向成组评测", pages:"25"},
  {type:"process", title:"AI产品评测的是完整版本组合", lead:"任何一层变化，都可能改变最终输出。", items:["模型版本","System Prompt","知识库","工作流","工具/API"], footer:"评测结论必须绑定版本，而不是只写“模型效果不错”。", pages:"25–27"},
  {type:"compare", title:"三个层次，回答三个不同问题", leftTitle:"功能验收", left:["流程能不能走通","接口与权限是否正常"], rightTitle:"离线评测 × 线上监控", right:["固定样本上效果如何","真实流量中是否持续稳定"], accent:"上线前测得准，上线后看得住", pages:"27–29"},
  {type:"map", title:"今天沿着一条产品闭环推进", items:["风险动机","Eval体系","Bad Case与回归","数据反馈闭环"], pages:"9、24、72"},
  {type:"statement", title:"幻觉不是“偶尔说错”", lead:"当答案进入客服、营销、医疗、金融或企业流程，错误会变成业务不可控。", accent:"可接受的产品，不等于永不出错；而是错误可发现、可拦截、可追踪。", risk:true, pages:"17、20"},
  {type:"bullets", title:"一次错误，也可能成为一次事故", lead:"错误的严重度，不由“错误率”单独决定。", items:["高频问题答错：持续侵蚀信任","低频高风险问题答错：直接触发合规或资金损失","语气看似确定：让用户更难识别风险"], pages:"18"},
  {type:"three", title:"三类典型幻觉，分别怎么错", items:[{h:"事实幻觉",t:"编造不存在的政策、价格、数据或出处"},{h:"指令幻觉",t:"没有遵守限制条件，擅自承诺或执行"},{h:"上下文幻觉",t:"忽略当前会话、用户身份或前置条件"}], pages:"19"},
  {type:"statement", title:"商业产品最怕的，是不知道何时会错", lead:"如果错误不可预测、不可观测、不可复现，团队就无法决定是否上线。", accent:"评测的价值：把“感觉不稳定”变成可讨论的证据。", risk:true, pages:"20"},
  {type:"process", title:"五道防线：让风险逐层收敛", items:["数据与知识","Prompt与规则","模型与工具","输出校验","人工兜底"], footer:"本课聚焦：如何用评测判断这些防线是否真的有效。", pages:"21"},
  {type:"bullets", title:"客服场景，先列风险再设计用例", items:["虚构退款、促销或物流政策","未核验身份就泄露订单信息","越权承诺赔付或到账时间","情绪激化、攻击性或歧视性表达","没有答案时仍强行作答"], pages:"22"},
  {type:"three", title:"三个常见误区，会让评测失真", items:[{h:"只测正常问题",t:"边界、异常和高风险样本被遗漏"},{h:"只看平均分",t:"严重错误被大量普通样本稀释"},{h:"改完只测原题",t:"修复一个问题，却可能伤到其他能力"}], pages:"23"},
  {type:"statement", title:"不能保证永不犯错，就要让错误可测", lead:"接下来把风险变成样本、指标、门槛和回归机制。", accent:"从“主观体验”进入“Eval体系”", pages:"23–24"},

  {type:"section", no:"01", title:"构建AI产品 Eval 体系", sub:"固定题、统一判卷、重复运行", pages:"24"},
  {type:"compare", title:"AI测试与传统测试，差异在“答案空间”", leftTitle:"传统软件", left:["输入—输出相对确定","Pass / Fail 边界清晰","相同版本容易复现"], rightTitle:"AI产品", right:["同一输入可能多种合理答案","质量标准包含主观判断","模型与上下文让结果波动"], pages:"25"},
  {type:"four", title:"AI测试有四个核心难点", items:["输出不确定","标准不唯一","覆盖成本高","版本变化快"], footer:"解决方案不是找一个万能指标，而是建立组合评测系统。", pages:"26"},
  {type:"six", title:"完整测试地图：六个观察视角", items:["功能","AI任务效果","可用性","非功能","鲁棒性","安全与伦理"], pages:"27"},
  {type:"statement", title:"本课聚焦两件事：功能与AI任务效果", lead:"先确保流程可用，再判断回答是否正确、相关、完整、安全、符合风格。", accent:"其他测试类型最后统一速览", pages:"28"},
  {type:"process", title:"三种执行方式，覆盖不同成本与规模", items:["用户场景模拟","人工抽测","自动化脚本"], footer:"小规模靠人工理解，大规模靠自动化复现；两者必须互相校准。", pages:"29"},
  {type:"three", title:"评测最难的，不是“跑题”，而是“判题”", items:[{h:"非确定",t:"同一题多次结果不完全相同"},{h:"主观标准",t:"正确之外，还有语气、风格和完整性"},{h:"规模覆盖",t:"真实用户表达远多于测试人员能穷举的范围"}], pages:"30"},
  {type:"statement", title:"量化不是为了制造一个漂亮分数", lead:"量化的作用，是让版本可比较、问题可定位、上线门槛可执行。", accent:"没有统一尺子，就没有可靠回归。", pages:"32"},
  {type:"process", title:"先冻结评测对象，再讨论结果", items:["记录版本组合","锁定评测集","统一评分规则","重复运行","比较差异"], footer:"每次报告都要能回答：测的是哪个版本？用的哪套题？谁来判？", pages:"46–50"},
  {type:"bullets", title:"评测集从四个入口持续生长", items:["真实用户问题：最接近线上分布","历史 Bad Case：最值得回归","业务规则与红线：最不能犯错","AI扩写变体：提高表达覆盖"], pages:"47、54"},
  {type:"five", title:"一套有用的题库，至少覆盖五类样本", items:["典型","边界","异常","无答案","高风险"], footer:"比例不必平均；应按业务频率与风险加权。", pages:"47"},
  {type:"table", title:"每条评测用例，建议保留八个字段", headers:["字段","作用"], rows:[["Case ID / 场景","确保可追踪"],["用户输入 / 上下文","还原真实条件"],["参考答案 / 评分点","明确判题依据"],["风险等级 / 标签","支持加权与筛选"],["版本 / 实际输出","支持回归比较"]], pages:"47–50"},
  {type:"case", title:"示例：促销叠加规则怎么变成评测用例", q:"用户：会员券能和满减活动同时使用吗？", a:"评分点：说明适用条件；不得承诺所有门店通用；信息不足时引导核验活动页。", tags:["业务规则","高频","中风险"], pages:"47–49"},
  {type:"statement", title:"语义相似度，解决“说法不同但意思相近”", lead:"它比较答案在语义空间中的距离，而不是逐字比对。", accent:"适合判断表达接近度，不适合单独判断事实真伪。", pages:"31、33"},
  {type:"visual", title:"余弦相似度：看两个答案指向是否接近", lead:"方向越接近，相似度越高；但“方向接近”不代表关键事实一定正确。", metric:"0.86", metricLabel:"语义接近", pages:"33"},
  {type:"case", title:"相似度演示：不同措辞，也可能是同一答案", q:"参考：订单付款后24小时内可以申请取消。", a:"AI：支付完成后的1天内，用户仍可发起取消申请。", tags:["核心条件一致","表达不同","高相似"], pages:"34–38"},
  {type:"compare", title:"高相似，不等于高正确", leftTitle:"参考答案", left:["退款到账：3—5个工作日"], rightTitle:"AI答案", right:["退款到账：5—7个工作日","其余表达几乎完全相同"], accent:"一个关键数字错误，足以让答案不可用", risk:true, pages:"40–41"},
  {type:"four", title:"四种评测方法，要组合而不是站队", items:["精确匹配 / 规则","语义相似度","LLM-as-Judge","人工评审"], footer:"客观字段用规则，开放答案用语义与裁判，关键决策由人工校准。", pages:"42、49"},
  ...
  {type:"five", title:"输出质量，至少拆成五个维度", items:["正确","相关","完整","安全","风格"], footer:"不同业务可以增删维度，但必须写清定义、评分点和红线。", pages:"48"},
  {type:"table", title:"不同维度，匹配不同判分方式", headers:["维度","优先方法","提醒"], rows:[["正确性","规则＋人工/LLM","核对关键事实与数字"],["相关性","语义＋LLM","避免答非所问"],["完整性","评分点覆盖","检查必要步骤"],["安全性","规则＋人工","红线单独统计"],["风格","LLM＋抽检","统一语气与格式"]], pages:"49、59"},
  {type:"statement", title:"自动化评测的意义，是把回归成本降下来", lead:"当题库从20条增长到2000条，人工逐题检查不再可持续。", accent:"自动化负责规模，人工负责标准和校准。", pages:"44–45"},
  {type:"visual", title:"什么是 Eval？", lead:"固定一套题，用统一规则判卷，并在每次版本变化后重复运行。", metric:"同题", metricLabel:"同尺 · 同流程", pages:"46"},
  {type:"process", title:"Eval第一步：设计代表性评测集", items:["按业务场景分层","加入边界与无答案","保留历史 Bad Case","标记频率与风险"], footer:"题库代表什么，最终分数就代表什么。", pages:"47"},
  {type:"process", title:"Eval第二步：定义评分点和红线", items:["写清理想答案","拆出必含信息","定义可接受变体","列出一票否决项"], footer:"先写判卷规则，再看模型答案，避免事后迁就。", pages:"48"},
  {type:"process", title:"Eval第三步：选择评分方法", items:["规则判硬条件","相似度看语义","LLM判开放质量","人工做终审校准"], footer:"组合方法比单一总分更可信。", pages:"49"},
  {type:"four", title:"一个自动评测中心，需要四个核心区", items:["用例库","批量运行","评分与归因","版本对比"], footer:"产品经理不一定写平台，但必须定义平台要回答的问题。", pages:"50"},
  {type:"demo", title:"演示：一次批量测试，应该看到什么", items:["选择版本与评测集","批量发送请求并留存输出","按维度自动评分","筛选低分与红线用例","生成版本对比报告"], pages:"51–52"},
  {type:"process", title:"自动测试 Agent：把执行、判分、报告串起来", items:["Test Case","被测产品","Judge","结果库","报告"], footer:"关键不是“用了Agent”，而是过程可重复、结果可审计。", pages:"53"},
  {type:"process", title:"AI可以扩写用例，但不能替你定义边界", items:["给定原始样本","指定变化维度","生成表达变体","人工抽检去重","加入候选题库"], footer:"扩写用于覆盖，不用于凭空创造业务规则。", pages:"54"},
  {type:"case", title:"Judge Prompt要把“好不好”写成判分规则", q:"请按正确性、完整性、安全性分别评分，并引用答案证据。", a:"红线：若虚构政策、越权承诺或泄露隐私，则安全性直接为0，并标记原因。", tags:["维度定义","评分锚点","输出JSON"], pages:"55–57"},
  {type:"three", title:"LLM裁判也有三个盲点", items:[{h:"偏好漂移",t:"模型或Prompt变化会改变评分尺度"},{h:"位置与措辞偏差",t:"更长、更像参考答案的回答可能被高估"},{h:"自评偏差",t:"同源模型可能偏爱自己的表达方式"}], pages:"55–57"},
  {type:"statement", title:"人工评审不是最后补一眼，而是校准系统", lead:"定期抽取自动高分、低分和争议样本，与人工金标准比较。", accent:"当裁判偏了，先修评分体系，再讨论被测模型。", pages:"58"},
  {type:"table", title:"报告不要只有一个平均分", headers:["层级","必须回答"], rows:[["总体","通过率、红线数、核心维度"],["场景","哪个场景退化或提升"],["问题","低分样本为什么失败"],["版本","相对基线变化多少"],["决策","上线、灰度、退回或转人工"]], pages:"60–62"},
  {type:"four", title:"评测成熟度：从抽查走向持续回归", items:["L1 人工抽查","L2 固定题库","L3 自动评测","L4 线上反馈驱动"], footer:"成熟度提升的标志：结果越来越可复现，反馈越来越快进入下一轮。", pages:"60–62"},
  {type:"four", title:"PM的职责：定范围、定标准、定门槛、推闭环", items:["测什么","怎样算好","何时能上线","失败如何进入优化"], footer:"评测不是测试同学的收尾工作，而是产品定义的一部分。", pages:"63"},

  {type:"two", title:"六类测试的其余部分：可用性与体验", leftTitle:"可用性", left:["任务能否完成","提示与反馈是否清楚","失败后能否恢复"], rightTitle:"用户体验", right:["等待感与节奏","结果可理解、可编辑","信任与控制感"], pages:"65–66"},
  {type:"three", title:"还要关注：非功能、鲁棒性、安全与伦理", items:[{h:"非功能",t:"延迟、成本、并发、稳定性"},{h:"鲁棒性",t:"错别字、噪声、提示攻击、异常输入"},{h:"安全与伦理",t:"隐私、偏见、越权、合规与可追责"}], pages:"67–71"},
  {type:"section", no:"02", title:"从评测结果到数据反馈闭环", sub:"让真实使用持续改进下一版", pages:"72"},
  {type:"statement", title:"数据不是库存，而是持续学习的燃料", lead:"真正的壁垒，不只是拥有数据，而是能把反馈稳定地转成更好的产品。", accent:"采集—理解—修复—验证—再上线", pages:"73"},
  {type:"four", title:"AI产品数据，通常来自四个入口", items:["业务原始数据","用户输入与输出","显性反馈","隐性行为"], footer:"采集必须满足隐私、授权、最小必要和可追踪。", pages:"74–75"},
  {type:"process", title:"数据飞轮：每一轮都回到评测", items:["采集","清洗与标注","问题归因","产品优化","回归评测"], footer:"没有回归验证的“数据优化”，只是未经证明的修改。", pages:"76"},
  {type:"compare", title:"显性反馈告诉你态度，隐性反馈告诉你行为", leftTitle:"显性反馈", left:["点赞 / 点踩","原因选择","文字评价"], rightTitle:"隐性反馈", right:["复制、收藏、分享","重生成、改写、放弃","转人工、任务完成"], pages:"77–79"},
  {type:"process", title:"作业类产品：记录用户怎样处理AI结果", items:["直接采用","部分采用","大幅修改","弃用"], footer:"修改前后差异，比一个“满意/不满意”更接近真实质量。", pages:"80–81"},
  {type:"bullets", title:"行为反馈常常比口头评价更诚实", items:["收藏：结果具有复用价值","重生成：当前答案未满足需求","放大/细看：内容值得进一步判断","复制/导出：可能进入真实工作流","迅速退出：可能无价值，也可能已完成任务"], pages:"82–83"},
  {type:"process", title:"客服产品：把反馈放在任务节点上", items:["回答后轻量反馈","失败原因快捷选择","低置信度主动转人工","人工处理结果回流","关闭会话前确认解决状态"], footer:"反馈机制要低摩擦，否则只会收集到极端意见。", pages:"84"},
  {type:"process", title:"低分会话，如何进入 Bad Case 问题池", items:["触发低分/红线","去敏与去重","人工复核","归因与定级","加入回归集"], footer:"不是每个点踩都是模型问题；先确认用户意图与业务规则。", pages:"85–86"},
  {type:"six", title:"Bad Case六类归因：找到真正该改的层", items:["数据","Prompt","知识","流程","工具","产品边界"], footer:"归因错了，越优化越乱。", pages:"85–95"},
  {type:"three", title:"标注规范必须同时给出三样东西", items:[{h:"定义",t:"标签到底描述什么，不描述什么"},{h:"正例",t:"哪些样本应该被标记"},{h:"反例",t:"相似但不应标记的边界样本"}], pages:"87–90"},
  {type:"process", title:"标注结果，最终要回到产品实验", items:["人工标注","问题聚类","方案设计","Prompt / 流程 A/B","Eval验证"], footer:"优化目标应对应具体失败类型，而不是笼统追求“更聪明”。", pages:"91–95"},
  {type:"process", title:"版本回归：修复原错，也保护原本正确的能力", items:["原失败样本","同类高风险样本","代表性正常样本","历史关键 Bad Case"], footer:"新版本只有同时通过修复验证与防退化验证，才有资格进入灰度。", pages:"60–63、91–95"},
  ...
  {type:"close", title:"评测的终点，是做出有依据的产品决策", items:["上线","灰度","退回优化","转人工兜底"], footer:"固定评测集 × 组合评分 × Bad Case × 版本回归 × 数据反馈闭环", pages:"60–63、72–101"},
];
